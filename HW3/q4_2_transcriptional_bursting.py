...
l_avg = np.zeros(n_t)
l_var = np.zeros(n_t)
raw_res = [simulate(l_init, T, beta_start, beta_stop, beta_s, k_sim, number_of_genes, n0, split_rate) for _ in range(n)]
...
log_zero_copy_p = np.log(zero_copy_p)
log_zero_copy_p_th = - beta_start * t_s

plt.figure()
plt.plot(panelC[:, 0], panelC[:, 1], marker="+", linestyle="", label=r'Experimental')
plt.plot(np.log10(l_avg[-1]), np.log10(l_var[-1]), marker="o", color="r", linestyle="", label=r'Simulation')
plt.xlabel(r'$\log_{10}\left\langle \ell_{\infty}\right\rangle $')
plt.ylabel(r'\log_{10}\text{var}\left(\ell_{\infty}\right)')
plt.legend()
...
plt.plot(t_s, log_zero_copy_p, label='simulation')
plt.plot(t_s, log_zero_copy_p_th, label='theory')
plt.xlabel('$t$ [min]')
